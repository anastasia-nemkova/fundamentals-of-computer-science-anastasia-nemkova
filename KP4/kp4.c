...
double second_derivative_F1(double x) {
    return -(cos(x) / (2 * cos(x / 2) * cos(x / 2) * cos(x / 2)) * sin(x / 2) * sin(x / 2) * sin(x / 2));
}

double second_derivative_F2(double x) { 
    return -((1 + 3 * x) / (sqrt(x) * (2 * sqrt(x) + 2 * sqrt(x) * fabs(x)) * (2 * sqrt(x) + 2 * sqrt(x) * fabs(x))));
}
